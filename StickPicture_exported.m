    ...
    methods (Access = private)
        ...
        function OpenFile(app)
            ...
            if result == 1
                ...
                p_tbl.name = tmp_tbl.name;
                
                tmp_tbl = readtable(filename,'Sheet','line');
                l_tbl.list = tmp_tbl.list;
                l_tbl.line = tmp_tbl.line;
                l_tbl.type = tmp_tbl.type;
                l_tbl.color = tmp_tbl.color;
                l_tbl.width = tmp_tbl.width;
                
                tmp_tbl = readtable(filename,'Sheet','dot');
                d_tbl.list = tmp_tbl.list;
                d_tbl.name = tmp_tbl.name;
                d_tbl.marker = tmp_tbl.marker;
                d_tbl.size = tmp_tbl.size;
                d_tbl.edge = tmp_tbl.edge;
                d_tbl.face = tmp_tbl.face;
                ...
                % リストファイルがない場合
                for ipnt = 1:np
                    p_tbl.point(ipnt) = ipnt;
                    p_tbl.name(ipnt) = ["P" + string(ipnt)];
                    d_tbl.name(ipnt) = ["P" + string(ipnt)];
                    d_tbl.marker(ipnt) = "円";
                    d_tbl.size(ipnt) = 6;
                    d_tbl.edge(ipnt) = "なし";
                    d_tbl.face(ipnt) = "白";
            ...
            %% ビューの設定
            app.UIAxes.Interactions = [rotateInteraction];  % 回転のみ操作を許可する
        ...
        function [ln,dot] = ConvertLine(app)
            ...
            if nline > 0
                for iline = 1:nline
                    ...
                    p_id = [];
                    for isplit = 1:nsplit
                        p = string(dat_split(isplit));
                        for irow = 1:height(p_tbl)
                            tmp_name = string(pname(irow));
                            if p == tmp_name
                                point_no = irow;
                                break;
                            else
...
%             ln = app.ln
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            app.FrameSpinnerLabel.Enable = 'off';
